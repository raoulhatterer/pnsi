...
def define_env(env):
    ...
    def rel_to_abs(chemin_relatif):
        ...
        Prends en compte le fichier markdown source, au build
        - `index.md` reste en place pour `index.html` créé
        - `autre.md` est propulsé dans `autre/index.html`
        
        Exemple, avec un fichier markdown source dans `dossier_source`
        
        rel_to_abs(script/test.py)
            renvoie
        'docs/dossier_source/script/test.py'
        
        """
        sources_fixes = ['index.md', ...]
        chemin_absolu = env.variables.page.file.dest_path.rstrip('/index.html')
        if env.variables.page.file.src_path.split('/')[-1] not in sources_fixes:
            chemin_absolu = os.path.dirname(chemin_absolu)
        if chemin_absolu == '':
            return f"docs/{chemin_relatif}"
        else:
            return f"docs/{chemin_absolu}/{chemin_relatif}"
